...
# TF_CPP_MIN_LOG_LEVEL = "2" to ignore warnings and only show errors.
...
# Fetch Bitcoin data
data = pd.DataFrame(yf.download("BTC-USD", start="2014-01-01", end=datetime.date.today())["Adj Close"])
# print(data) # check
...
# lag = 17 
# as forecasting = 30, we can revise lag to 30
...
model.fit(x_train, y_train, epochs=250, validation_data=(x_test, y_test), callbacks=[es], verbose=1)
